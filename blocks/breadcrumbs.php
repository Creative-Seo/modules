...
{
	if(!(isset($breadcrumb2_url))) {$breadcrumb2_url = HTTP_SERVER.$self[count($self)-2];}
...
{
	if(!(isset($breadcrumb3_url))) {$breadcrumb3_url = HTTP_SERVER.$self[count($self)-3].'/'.$self[count($self)-2];}
...
?>      
        <span class="active" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">
			<a href="<?php echo "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];?>" itemprop="url">
